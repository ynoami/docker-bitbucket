...
if [ "${SSL}" = "TRUE" ]; then
 isMod=$(grep "SSLCONNECTOR_INSERT" -c /opt/atlassian/atlassian-bitbucket-${BITB_VERSION}/conf/server.xml)
 ...
 sed -i 's|</Service>|<Connector port="8443" protocol="org.apache.coyote.http11.Http11Protocol" maxHttpHeaderSize="8192" \
  ...
  /opt/atlassian/atlassian-bitbucket-${BITB_VERSION}/conf/server.xml
  echo "SSL connector is registered"
else
 echo "SSL=FALSE"
fi

/opt/atlassian/atlassian-bitbucket-${BITB_VERSION}/bin/start-bitbucket.sh -fg
